...
def _do_transcribe(audio_path: str, language: str = "auto", model_name: str = "sensevoice",
                   speaker_diarization: bool = False) -> str:
    """内部统一的转写函数"""
    registry = ModelRegistry.get_instance()
    model_key = "sensevoice_spk" if speaker_diarization else model_name
...
def transcribe_audio(audio_path: str, language: str = "auto",
    ...
    Args:
        ...
        model: 模型选择（sensevoice/paraformer/fun-asr-nano），默认 sensevoice
        speaker_diarization: 是否启用说话人分离，默认关闭
    """
    if not os.path.exists(audio_path):
        return f"错误：文件不存在: {audio_path}"
    return _do_transcribe(audio_path, language=language, model_name=model,
...
def transcribe_url(url: str, language: str = "auto",
    ...
    try:
        return _do_transcribe(tmp_path, language=language, model_name=model,
                              speaker_diarization=speaker_diarization)
    finally:
        os.unlink(tmp_path)
